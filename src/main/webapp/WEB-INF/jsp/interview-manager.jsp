...
<script>
	...
	$(function(){
		$.get("../ae/interview/get/invated/et/"+eid+"/0/15",function(data){
			document.getElementById("recruit-msg").innerHTML =""
        		var list=data.data;
    			for(var i in list){
    				$("#recruit-msg").append("<div style='border: 1px solid #e3c56b;width:200px;position: absolute;left:"+(38+250*(i%3))+"px;top:"+(15+85*parseInt(i/3))+"px;height:80px;font-size: 15px;'><div width='180px'  style='position: relative;'><p style='margin-left: 10px;margin-top:10px;'>面试者："+list[i].name+"</p><p hidden>"+list[i].qid+"</p><p style='margin-left: 10px;'>面试职业:<a plain='true' class='quarter'>"+list[i].quartername+"</a></p><span style='position: relative;top:-70px;left:130px;'><b><p hidden>"+list[i].aid+"</p><a plain='true' class='checkoffice'>查看简历</a></b></span></div></div>");
    			}
    			$(".quarter").linkbutton();
    			...
    			$(".checkoffice").bind('click', function(){
    				window.location.href = "/ae/resume/msg2/"+$(this).prev().text()+"/"+$(this).parent().parent().prev().prev().text();
    			})
    	});
		...
		$("#pagination").pagination({
	        ...
	        onSelectPage: function (pageNumber, pageSize) {
	        	document.getElementById("recruit-msg").innerHTML =""
	        	$.get("../ae/interview/get/invated/et/"+eid+"/"+(pageNumber-1)*8+"/"+pageSize,function(data){
	    			document.getElementById("recruit-msg").innerHTML =""
	            		var list=data.data;
	        			for(var i in list){
	        				$("#recruit-msg").append("<div style='border: 1px solid #e3c56b;width:200px;position: absolute;left:"+(38+250*(i%3))+"px;top:"+(15+85*parseInt(i/3))+"px;height:80px;font-size: 15px;'><div width='180px'  style='position: relative;'><p style='margin-left: 10px;margin-top:10px;'>面试者："+list[i].name+"</p><p hidden>"+list[i].qid+"</p><p style='margin-left: 10px;'>面试职业:<a plain='true' class='quarter'>"+list[i].quartername+"</a></p><span style='position: relative;top:-70px;left:130px;'><b><p hidden>"+list[i].aid+"</p><a plain='true' class='checkoffice'>查看简历</a></b></span></div></div>");
