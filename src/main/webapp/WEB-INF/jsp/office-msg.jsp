	    ...
	    <div style="width:auto;position: relative;" data-options="region:'center'" >
			...
			<div id="centerpanel" class="easyui-panel" style="width:auto;position: absolute;top:30px;left:0px;border:0;">
				<div style="position: relative;height:850px;margin-left: 60px;">
					<table cellpadding="5">
						<tr><th style="color: red;font-size: 20px;width:100px;">基本信息</th></tr>
						...
						<tr><th style="color: red;font-size: 20px;">工作经验/社会实践</th></tr>
					...
					<div style="font-size: 15px;">
	            		<button style="margin-left:10%;margin-right:20px;background-color: #76EE00;" onclick="accept()">接受简历</button>
	   					<button style="margin-left:20px;background-color: #FF3030;" onclick="refuse()">拒绝简历</button>
            		</div>
            	</div>
...
<script>
	...
	function accept(){
		...
		var qid=${qid};
		$.get("/ae/resume/acceptresume/"+aid+"/"+qid,function(data){
			if(data.status == 200){
				alert("接受成功");
	...
	function refuse(){
		var aid=${applicant.aid};
		var qid=${qid}
		$.get("/ae/resume/refuseresume/"+aid+"/"+qid,function(data){
			if(data.status == 200){
				alert("拒绝成功");
				window.location.href = "/euser/office-manager";
			}
